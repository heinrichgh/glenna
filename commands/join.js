                ...
                if (collected.first().content.trim() == "yes")
                {
                    ...
                    if (sql_rank_id[0]) {
                        ...
                        if (sql_member_id[0])
                            ...
                            await sql.execute('INSERT INTO `guild_member` (`id`, `guild_id`, `guild_member_name`, `discord_id`, `rank_id`, `api_key`) VALUES (?, ?, ?, ?, ?, ?)',[null, sql_rank_id[0].guild_id, account.name, this.message.author.id, sql_rank_id[0].id, this.args[0]]);
                    }
